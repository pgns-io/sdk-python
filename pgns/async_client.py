...
class AsyncPigeonsClient:
    ...
    async def get_artifact(
        self, artifact_id: str, *, token: str | None = None
    ) -> tuple[bytes, str]:
    ...
    async def list_artifacts(
        ...
        *,
        ...
        resp = await self._raw_request("GET", "/v1/artifacts", params=params or None)
        raw = _handle_response(resp)
        return PaginatedArtifacts.model_validate(raw)

    async def delete_artifact(self, artifact_id: str) -> None:
        """Delete an artifact."""
        await self._request("DELETE", f"/v1/artifacts/{quote(artifact_id, safe='')}")

    # -- Sagas ----------------------------------------------------------------

    async def list_sagas(self) -> list[Saga]:
        """List all saga definitions for the authenticated user."""
        data = await self._request("GET", "/v1/sagas")
        return [Saga.model_validate(s) for s in data]

    async def create_saga(self, data: CreateSaga) -> Saga:
        """Create a new saga definition."""
        raw = await self._request("POST", "/v1/sagas", json=data.model_dump(exclude_none=True))
        return Saga.model_validate(raw)

    async def get_saga(self, saga_id: str) -> Saga:
        """Get a saga by ID."""
        raw = await self._request("GET", f"/v1/sagas/{quote(saga_id, safe='')}")
        return Saga.model_validate(raw)

    async def get_saga_by_name(self, name: str) -> Saga:
        """Get a saga by name."""
        raw = await self._request("GET", f"/v1/sagas/by-name/{quote(name, safe='')}")
        return Saga.model_validate(raw)

    async def delete_saga(self, saga_id: str) -> None:
        """Delete a saga definition."""
        await self._request("DELETE", f"/v1/sagas/{quote(saga_id, safe='')}")

    async def execute_saga(self, saga_id: str, data: ExecuteSagaRequest) -> SagaInstance:
    ...
    async def list_saga_instances(
        ...
        *,
        ...
        data = await self._request("GET", f"/v1/sagas/{quote(saga_id, safe='')}/instances{qs}")
        return [SagaInstance.model_validate(i) for i in data]

    async def get_saga_instance(self, saga_id: str, instance_id: str) -> SagaInstanceDetail:
